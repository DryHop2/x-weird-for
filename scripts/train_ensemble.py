...
def main():
    ...
    parser.add_argument("--output", default="models/ensemble.pkl", help="Ouput file")
    ...
    if isinstance(data, list) and "headers" in data[0]:
    ...
    models = train_ensemble(features, args.conamination)
    ...
    for name, model in models:
        ...
        print(f"\n{name.upper()} model:")
        print(f"  Anomalies detected: {sum(model.predict(features) == -1)}/{len(features)}")
